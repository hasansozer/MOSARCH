    ...
    for l in range(num_clusters):
        ...
        for i in modulesOnClusters[l]:
            for j in modulesOnClusters[l]:
                if i!=j:
                    modularity += (dependency_matrix[i][j]-(d_out[i]*d_in[j]/num_dependencies))/num_dependencies
    return modularity
...
def Mutation(parent,inputdata):
    ...
    if np.random.random() <= 1.00000:
        ...
        if RAND<-0.2:
            ...
            child[0]=np.insert(child[0],np.random.choice(len(child[0])),np.random.randint(0, num_clusters))
        elif 22.2<=RAND<25.4:
